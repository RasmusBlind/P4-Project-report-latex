...
\section{Results}
	...
	\subsection{Mel Frequency Cepstrum Coefficients}
		...
		Over all three configurations of the MFCC feature vector, the best performing were $K=7$ and $K=8$, when using a window size of 5ms and skip of 2ms, see table \ref{table:eval:mfccBest1}) and \ref{table:eval:mfccBest2}. Although some of other results with the same parameters reach similar performance in regards to precision per class (see appendix \ref{tlmfcc52}). The worst performing, by accuracy, is using 20ms window size and 10ms skip with $K=2$ (see table \ref{table:eval:mfccWorst}). It still has better precision for 'k' than some others(e.g. $K=4$ or $K=6$), but some have similar 'hh' precision (e.g. $K=1$ or $K=6$). All configurations, both in parameters and K, disproved the null hypothesis with $alpha=0.01$.
